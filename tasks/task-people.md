...
- `P02 | TODO | Run manual smoke tests on the dev machine for task CRUD, chat, reminders, subtitles, and offline mode. | Done when: the main user flows have been exercised manually and the results are written down for follow-up. | Unblocks: A03 A04 A07 A09 A10 A11 A14 A15`
- `P03 | TODO | Install and configure Whisper.cpp, Piper, and Ollama with real model and path settings on the demo machine. | Done when: the machine has working local runtime paths and models that the app can point at for end-to-end testing. | Unblocks: A03 A04 A07 A10 A11 A12 A13 A14 A15`
...
Quy ước: `Pxx` = công việc người dùng phải làm thủ công/ngoài repo | `Axx` = công việc Codex/AI có thể thực hiện trong repo, được theo dõi trong `tasks/task-queue.md`
...
- `P02 | TODO | Chạy manual smoke test trên máy dev cho task CRUD, chat, reminder, subtitle, và offline mode. | Done when: các luồng chính đã được kiểm tra thủ công và kết quả đã được ghi lại để theo dõi. | Unblocks: A03 A04 A07 A09 A10 A11 A14 A15`
- `P03 | TODO | Cài đặt và cấu hình Whisper.cpp, Piper, và Ollama với model/path thật trên máy dùng để demo. | Done when: máy đã có các runtime local, model, và đường dẫn cấu hình đúng để app có thể chạy end-to-end. | Unblocks: A03 A04 A07 A10 A11 A12 A13 A14 A15`
- `P04 | TODO | Cung cấp avatar asset thật, animator controller, và kỳ vọng về blendshape/lip-sync. | Done when: repo đã có đầu vào avatar production cần thiết để thay thế placeholder một cách an toàn. | Unblocks: A03 A04 A07 A10 A11 A14 A15 A19`
